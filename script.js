...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentCategory = 'General'; // Actualiza currentCategory a 'General' al cargar la página
    localStorage.setItem('currentCategory', 'General');
        ...
        cards[currentCategory].forEach((card, index) => {
            ...
            if (card.link.startsWith('@')) {
                cardElement.addEventListener('click', () => {
                    const confirmOpen = confirm('¿Estás seguro de que deseas abrir esta URL?');
                    if (confirmOpen) {
    ...
    const editCategory = (categoryName, index) => {
        ...
        if (newCategoryName && !categories.includes(newCategoryName)) {
            // Conservar las tarjetas asociadas a la categoría
            cards[newCategoryName] = cards[categoryName];
            delete cards[categoryName];
    
            // Cambiar el nombre de la categoría en el array y en localStorage
    ...
    const saveData = () => {
        ...
        const exportToCSV = () => {
            ...
            // Iterar sobre las tarjetas y agregarlas al contenido CSV
            ...
            const formattedDate = `${day}-${month}-${year} ${hours}.${minutes}`;
            downloadLink.download = `My Walllist - ${formattedDate}.csv`;
        ...
        // Agregar evento al botón de exportar datos CSV
